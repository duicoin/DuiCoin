...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64& nStakeModifier, int& nStakeModifierHeight, int64& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval)
    {
        ...
        {   // reached best block; may happen if node is behind on block chain
			...
			{
               //  printf(">> nStakeModifierTime = %"PRI64d", pindexFrom->GetBlockTime() = %"PRI64d", nStakeModifierSelectionInterval = %d,nStakeMinAge = %d, GetAdjustedTime() = %d\n",
               //     nStakeModifierTime, pindexFrom->GetBlockTime(), nStakeModifierSelectionInterval,nStakeMinAge,GetAdjustedTime());
                return false;
			}
        }
        pindex = pindex->pnext;
