...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "DuiCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
 ...
 printf("dss \n");
        for (unsigned int seed_idx = 0; seed_idx < ARRAYLEN(strDNSSeed); seed_idx++) {
           printf("dss \n");    if (HaveNameProxy()) {
                ...
                vector<CAddress> vAdd;       printf("ds \n");
...
//122.112.85.101
 unsigned int pnSeed[] = {
     0x35594A70,//112.74.89.53
    0x42E4387B,//118.123.11.245
    0x521DE567, /* IP addr 122.112.85.197  */
    0xAFD61955,
    0xF622C48B, /* IP addr 122.112.85.122  */
  };
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. DuiCoin is probably already running."), addrBind.ToString().c_str());
        else
